...
public class ChaosMonkeyResource {
    ...
    @Path("/Gorilla")
    @POST
    public Response unleashGorilla(String content) throws IOException {
    	ObjectMapper mapper = new ObjectMapper();
    	JsonNode input = mapper.readTree(content);
    	String elbName = getStringField(input, "elbName");
    	AWSClient client;
    	client = new AWSClient(Region.US_WEST_2);
    	LOGGER.info(String.format("Gorilla unleashed on ELB '%s'", elbName));
    	client.disableAvailabilityZonesForLoadBalancer(elbName);
